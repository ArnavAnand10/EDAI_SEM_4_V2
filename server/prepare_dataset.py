...
def preprocess_image(image):
    ...
    filtered_img = median(
        fingerprint,
        footprint=disk(1),  # small circular footprint for median filtering
        mode="nearest",
    )
    filtered_img_uint8 = (filtered_img * 255).clip(0, 255).astype(np.uint8)
    colored = cv2.cvtColor(filtered_img_uint8, cv2.COLOR_GRAY2BGR)
